...
FROM NhaCungCap
WHERE (@Keyword = N''
       OR MaNCC LIKE @LikeKeyword
       OR TenNCC LIKE @LikeKeyword
...
ORDER BY TenNCC;";
        ...
        private void ApplyPermissions()
        {
            ...
            txtMaNCC.ReadOnly = _mode != FormMode.Add;
        ...
        private bool ValidateInput()
        {
            ...
            if (_mode == FormMode.Add)
            {
                int duplicate = DbHelper.ExecuteScalar<int>(
                    "SELECT COUNT(1) FROM NhaCungCap WHERE MaNCC = @Ma;",
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            int productCount = DbHelper.ExecuteScalar<int>(
                "SELECT COUNT(1) FROM SanPham WHERE MaNCC = @Ma;",
                new SqlParameter("@Ma", ma));
            int importCount = DbHelper.ExecuteScalar<int>(
                "SELECT COUNT(1) FROM PhieuNhapKho WHERE MaNCC = @Ma;",
                new SqlParameter("@Ma", ma));
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else if (_mode == FormMode.Edit)
                {
                    DbHelper.ExecuteNonQuery(
                        @"UPDATE NhaCungCap
                          SET TenNCC = @Ten, DiaChi = @DiaChi, SoDienThoai = @SoDienThoai
                          WHERE MaNCC = @Ma;",
                        new SqlParameter("@Ma", txtMaNCC.Text.Trim()),
                        new SqlParameter("@Ten", txtTenNCC.Text.Trim()),
                        new SqlParameter("@DiaChi", string.IsNullOrWhiteSpace(txtDiaChi.Text) ? (object)DBNull.Value : txtDiaChi.Text.Trim()),
