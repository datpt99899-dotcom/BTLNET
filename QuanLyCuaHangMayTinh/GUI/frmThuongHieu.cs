...
namespace QuanLyCuaHangMayTinh_Forms.Forms
{
    public partial class frmThuongHieu : Form
    {
        ...
        private void frmThuongHieu_Load(object sender, EventArgs e)
        {
            ...
            // Wire SelectionChanged để btnSua/btnXoa enable lại khi chọn dòng (Designer quên)
            dgvDSThuongHieu.SelectionChanged += dgvDSThuongHieu_SelectionChanged;
...
ORDER BY TenThuongHieu;";
            ...
            try
            {
                dgvDSThuongHieu.DataSource = DbHelper.GetDataTable(
                    query,
                    new SqlParameter("@Keyword", keyword ?? string.Empty),
                    new SqlParameter("@LikeKeyword", $"%{keyword?.Trim()}%"));

                if (dgvDSThuongHieu.Columns.Count > 0)
                {
                    dgvDSThuongHieu.Columns["MaThuongHieu"].HeaderText = "Mã thương hiệu";
                    dgvDSThuongHieu.Columns["TenThuongHieu"].HeaderText = "Tên thương hiệu";
                }

                if (dgvDSThuongHieu.Rows.Count > 0)
                {
                    dgvDSThuongHieu.Rows[0].Selected = true;
                    FillInputsFromGrid();
                }
                else
                {
        ...
        private void dgvDSThuongHieu_SelectionChanged(object sender, EventArgs e)
        {
        ...
        {
            if (dgvDSThuongHieu.CurrentRow == null || dgvDSThuongHieu.CurrentRow.DataBoundItem == null)
                return;

            txtMaThuongHieu.Text = Convert.ToString(dgvDSThuongHieu.CurrentRow.Cells["MaThuongHieu"].Value);
            txtTenThuongHieu.Text = Convert.ToString(dgvDSThuongHieu.CurrentRow.Cells["TenThuongHieu"].Value);
        }
        ...
        private void ApplyPermissions()
        {
            ...
            btnSua.Enabled = _canManage && _mode == FormMode.View && dgvDSThuongHieu.CurrentRow != null;
            btnXoa.Enabled = _canManage && _mode == FormMode.View && dgvDSThuongHieu.CurrentRow != null;
        ...
        {
            if (dgvDSThuongHieu.CurrentRow == null)
            {
