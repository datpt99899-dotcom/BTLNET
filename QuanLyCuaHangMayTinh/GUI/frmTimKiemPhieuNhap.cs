...
namespace QuanLyCuaHangMayTinh.GUI
{
    ...
    public partial class frmTimKiemPhieuNhap : Form
    {
        ...
        private void FrmTimKiemPhieuNhap_Load(object sender, EventArgs e)
        {
            ...
            // button1 = "Tìm lại" / "Đóng" — wire để đóng form
            if (button1 != null) button1.Click += (s, ev) => ResetFilters();
            if (button2 != null) button2.Click += (s, ev) => this.Close();
        ...
        private void DgvDSPhieuNhap_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                // Hiển thị chi tiết phiếu nhập trong MessageBox (đơn giản, không phá Designer)
                string sql = @"
                    SELECT sp.MaSanPham, sp.TenSanPham, ct.SoLuong, ct.DonGiaNhap,
                           (ct.SoLuong * ct.DonGiaNhap) AS ThanhTien
                    FROM   ChiTietPhieuNhap ct
                    INNER JOIN SanPham sp ON ct.MaSanPham = sp.MaSanPham
                    WHERE  ct.MaPhieuNhap = @ma";
                var dt = Function.GetDataToTable(sql, new SqlParameter("@ma", maPhieu));

                if (dt.Rows.Count == 0)
                {
                    MessageBox.Show("Phiếu nhập này không có chi tiết sản phẩm.",
                        "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                // Build chi tiết hiển thị
                System.Text.StringBuilder sb = new System.Text.StringBuilder();
                sb.AppendLine("CHI TIẾT PHIẾU NHẬP: " + maPhieu);
                sb.AppendLine(new string('─', 60));
                decimal tong = 0;
                foreach (DataRow r in dt.Rows)
                {
                    string ma  = r["MaSanPham"].ToString();
                    string ten = r["TenSanPham"].ToString();
                    int sl     = Convert.ToInt32(r["SoLuong"]);
                    decimal gia= Convert.ToDecimal(r["DonGiaNhap"]);
                    decimal tt = Convert.ToDecimal(r["ThanhTien"]);
                    tong += tt;
                    sb.AppendLine($"  • {ma} — {ten}");
                    sb.AppendLine($"      SL: {sl}   |   Đơn giá: {gia:N0}   |   Thành tiền: {tt:N0}");
                }
                sb.AppendLine(new string('─', 60));
                sb.AppendLine($"TỔNG TIỀN: {tong:N0} VNĐ");

                MessageBox.Show(sb.ToString(), "Chi tiết phiếu nhập",
