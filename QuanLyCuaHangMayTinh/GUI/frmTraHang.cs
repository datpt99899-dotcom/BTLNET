...
namespace QuanLyCuaHangMayTinh
{
    public class frmTraHang : Form
    {
        ...
        {
            // Combo Mã nhân viên: load từ bảng NhanVien
            DataTable dtNV = Function.GetDataToTable("SELECT MaNhanVien, HoTen FROM NhanVien WHERE TrangThai = 1");
            cboMaNhanVien.DataSource = dtNV;
            cboMaNhanVien.DisplayMember = "MaNhanVien";
            cboMaNhanVien.ValueMember = "MaNhanVien";

            // Combo Mã khách: load từ bảng KhachHang (KHÔNG dùng dt nhân viên)
            DataTable dtKH = Function.GetDataToTable("SELECT MaKhachHang, TenKhachHang FROM KhachHang ORDER BY MaKhachHang");
            cboMaKhachHang.DataSource = dtKH;
            cboMaKhachHang.DisplayMember = "MaKhachHang";
            cboMaKhachHang.ValueMember = "MaKhachHang";

            // Fill text box tên ngay với giá trị đầu tiên
            if (dtNV.Rows.Count > 0)
                txtTenNhanVien.Text = dtNV.Rows[0]["HoTen"].ToString();
            if (dtKH.Rows.Count > 0)
                txtTenKhachHang.Text = dtKH.Rows[0]["TenKhachHang"].ToString();

            // Mặc định chọn NV đang đăng nhập nếu có trong list
            if (!string.IsNullOrEmpty(StaticData.MaNV))
            {
                int idxNV = cboMaNhanVien.FindStringExact(StaticData.MaNV);
                if (idxNV >= 0) cboMaNhanVien.SelectedIndex = idxNV;
            }
        }
        ...
        {
            // Khi chọn mã NV → load tên NV vào textbox
            if (cboMaNhanVien.SelectedValue == null) return;
            try
            {
                var dt = Function.GetDataToTable(
                    "SELECT HoTen FROM NhanVien WHERE MaNhanVien=@id",
                    new SqlParameter("@id", cboMaNhanVien.SelectedValue.ToString()));
                if (dt.Rows.Count > 0)
        ...
        {
            // Khi chọn mã KH → load tên KH vào textbox (KHÔNG đồng bộ với NV)
            if (cboMaKhachHang.SelectedValue == null) return;
            try
            {
                var dt = Function.GetDataToTable(
                    "SELECT TenKhachHang FROM KhachHang WHERE MaKhachHang=@id",
                    new SqlParameter("@id", cboMaKhachHang.SelectedValue.ToString()));
                if (dt.Rows.Count > 0)
